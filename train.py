...
import math
import torch
from WDR import calc_WDR
from utils.ValueCaculator import ValueCalculator2 as VC2
from utils.ValueCaculator import ValueCalculator1 as VC1
from utils.prioritized_memory import Memory, WeightedMSE
import sys
from os import path
...
class DeepQL(nn.Module):
    def __init__(self, Net, actionFinder=None, eps=0.9, lr=5e-3, gamma=0.9, mbsize=20, C=100, N=500, lambda1=1.0,
                 lambda2=1.0, lambda3=1e-5, n_step=3, demos=None):
        super().__init__()
        ...
        self.opt = optim.Adam(
            self.vc.predictNet.parameters(), lr=lr, weight_decay=lambda3)
    ...
    def storeDemoTransition(self, s, a, r, s_, done, demoEpisode):
        s = torch.Tensor(s)
        s_ = torch.Tensor(s_)
        # replay of certain demo episode
    ...
    def JE(self, samples):
        ...
        for s, aE, *_, isdemo in samples:
            ...
            # action with largest and second largest Q
            A1, A2 = np.array(A)[:2]
    ...
    def Jn(self, samples, Qpredict):
        ...
        for i, (s, a, r, s_, done, isdemo) in enumerate(samples):
            ...
            discountedR = reduce(lambda x, y: (
                x[0] + self.gamma ** x[1] * y, x[1] + 1), nr, (0, 0))[0]
    ...
    def update(self):
        ...
        for demo in self.demos:
            demo.prop.update(calc_WDR)
        demo_samples = int(self.mbsize * demo_prop)
        demo_trans = []
        if demo_samples > 0:
            demo_trans = self.demos.sample(demo_samples)
        agent_trans = self.replay.sample(self.mbsize - demo_samples)
        total_samples = demo_trans + agent_trans
        
        idxs, IS = self.sample()
        Qpredict, Qtarget = self.calcTD(total_samples)
        ...
        JE = self.JE(total_samples)
        Jn = self.Jn(total_samples, Qpredict)
